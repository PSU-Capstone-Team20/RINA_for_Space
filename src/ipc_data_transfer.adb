...
--     SDU_Length := Natural(Raw_Data(1));
--     End_Index :=  Start_Index + SDU_Length -1;

--     if Start_Index >= Raw_Data'First and End_Index <= Raw_Data'Last then 
--        SDU := Raw_Data(Start_Index .. End_Index);
--     else
--        SDU := (1 .. 0 => 0);
--     end if;
...
procedure Multiplex_PDU(PDU : in EFCP.PDU_S_T; Lower_Flow_ID : out Flow_ID) is
   Hash : Natural := 0;
begin
   for C of To_String(PDU.PCI.Src_CEP_ID) loop
      Hash := (Hash + CHaracter'Pos(C)) mod 10;
   end loop;
   Lower_Flow_ID := Flow_ID(Hash);
