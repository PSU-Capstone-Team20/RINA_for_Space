...
package body RINA.Policies.DTN_Bundle_Protocol is

   Min_Flow_ID   : constant Integer := 0;
   Max_Bundle_ID : constant Integer := 9_998;

   procedure Create_Bundle (Flow : in Flow_ID; Bundle : out Bundle_ID) is
   begin
      if Integer (Flow) < Min_Flow_ID then
         Put_Line
           (Integer'Image (Integer (Flow)) & " Flow ID is not allowed.");
         Bundle := 0;
      else
         Put_Line
           ("Creating a new Bundle for Flow " &
            Integer'Image (Integer (Flow)));
   ...
   begin
      if Integer (Flow) < Min_Flow_ID then
         ...
         Put_Line
           (Integer'Image (Integer (Bundle)) & " Bundle ID is out of bounds.");
      else
   ...
   begin
      if Integer (Flow) < Min_Flow_ID then
         Put_Line
           ("Error: Flow ID " & Integer'Image (Integer (Flow)) &
            " is not allowed.");
      else
         Put_Line
           ("Receiving a bundle from Flow " & Integer'Image (Integer (Flow)));
         Bundle := 2;
      end if;
   end Receive_Bundle;
   ...
   begin
      if Integer (Bundle) > Max_Bundle_ID then
         Put_Line
           ("Error: Bundle ID " & Integer'Image (Integer (Bundle)) &
            " is not allowed");
      else
         case Status is
            when Pending =>
               Put_Line
                 ("Custody for Bundle " & Integer'Image (Integer (Bundle)));
               Put_Line ("is: Pending");
            when Accepted =>
               Put_Line
                 ("Custody for Bundle " & Integer'Image (Integer (Bundle)));
...
end RINA.Policies.DTN_Bundle_Protocol;
