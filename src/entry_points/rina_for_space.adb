...
with Ada.Text_IO;           use Ada.Text_IO;
with RINA.Policies;         use RINA.Policies;
with RINA.Policies.DTN_Bundle_Protocol;

procedure Rina_For_Space is
   test1  : dif.DIF_Vector;
   num    : Integer;
   choice : Character;
   -- Flow              : RINA.Policies.Flow_ID := 1;
...
begin
   ...
   -- RINA.Policies.DTN_Bundle_Protocol.Create_Bundle(Flow, Bundle);
   -- RINA.Policies.DTN_Bundle_Protocol.Send_Bundle(Flow, Bundle);
   -- RINA.Policies.DTN_Bundle_Protocol.Receive_Bundle(Flow, Bundle);
   -- RINA.Policies.DTN_Bundle_Protocol.Handle_Custody(Bundle,RINA.Policies.DTN_Bundle_Protocol.Pending);

   -- Put_Line("RINA Policies test complete");
   -- Put_Line ("Initializing test");
   -- RINA.Policies.Define_QoS(Flow, QoS);
   -- RINA.Policies.Schedule_Flow(Flow);
   -- RINA.Policies.Relay_And_Forward (Source_Flow => Flow, Destination_Flow => 2);
   -- RINA.Policies.Handle_Error (Flow, Error_Code => 404);
      ...
      case choice is
         when '1' =>
            ...
            begin
               ...
               dif.createNamedDIF
                 (User_Input_ID, test1,
                  To_Unbounded_String (User_Input_Name (1 .. Last)));
               Put ("DIF Created with ID" & Integer'Image (User_Input_ID));
               Put_Line
                 (" and with name " &
                  String'Image (User_Input_Name (1 .. Last)));
            end;
         ...
         when '2' =>
            ...
            begin
               ...
               if Found then
                  ...
                  dif.createIPCP
                    (To_Unbounded_String (Name (1 .. Last)), test1 (Index));
                  Put_Line
                    ("IPCP created successfully in DIF with ID: " &
                     Integer'Image (DIF_ID));
               else
                  Put_Line
                    ("No matching DIF found with ID: " &
                     Integer'Image (DIF_ID));
               end if;
            end;
         ...
         when '4' =>
            for I in test1.First_Index .. test1.Last_Index loop
               ...
               Put_Line
                 (" " & Unbounded_String'Image (dif.getName (test1 (I))));
            end loop;
         ...
         when '7' =>
            ...
            begin
               ...
               dif.createNamedDIF
                 (2, test1, To_Unbounded_String ("Earth Network DIF"));
               dif.createNamedDIF
                 (3, test1, To_Unbounded_String ("Deep Space Relay DIF"));
               ...
               dif.createIPCP
                 (To_Unbounded_String ("Local Desktop"), test1 (Index));
               dif.createIPCP
                 (To_Unbounded_String ("NASA Server 1"), test1 (Index));
               ...
               dif.createIPCP
                 (To_Unbounded_String ("Madrid Ground Station"),
                  test1 (Index));
               dif.createIPCP
                 (To_Unbounded_String ("Goldstone Ground Station"),
                  test1 (Index));
               dif.createIPCP
                 (To_Unbounded_String ("Canberra Ground Station"),
                  test1 (Index));
               ...
               dif.createIPCP
                 (To_Unbounded_String ("Mars Odyssey"), test1 (Index));
               dif.createIPCP
                 (To_Unbounded_String ("Mars Reconnaissance Orbiter"),
                  test1 (Index));
         ...
         when '8' =>
            declare
               Flow   : RINA.Policies.Flow_ID                       := 1;
               Bundle : RINA.Policies.DTN_Bundle_Protocol.Bundle_ID := 1_000;
               QoS    : RINA.Policies.QoS_Parameter                 :=
                 (Priority => 1, Latency => 100, Throughput => 500,
                  QoS_ID   => 42);
            begin
               ...
               RINA.Policies.Define_QoS (Flow, QoS);
               RINA.Policies.Schedule_Flow (Flow);
               RINA.Policies.Relay_And_Forward
                 (Source_Flow => Flow, Destination_Flow => 2);
               RINA.Policies.DTN_Bundle_Protocol.Create_Bundle (Flow, Bundle);
               RINA.Policies.DTN_Bundle_Protocol.Send_Bundle (Flow, Bundle);
               RINA.Policies.DTN_Bundle_Protocol.Receive_Bundle (Flow, Bundle);
               RINA.Policies.DTN_Bundle_Protocol.Handle_Custody
                 (Bundle, RINA.Policies.DTN_Bundle_Protocol.Pending);
            end;
